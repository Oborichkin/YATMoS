def test_create_run(client, db_with_3_steps, db_with_3_projects):
    ...
    resp = client.post(f"/api/project/{x.id}/run", json={"title": "Nightly run", "desc": "00:00 everyday run"})
    ...
    resp = client.get(f"/api/project/{x.id}/runs")
    assert resp.status_code == 200, resp.reason
    ...
    resp = client.get(f"/api/run/{run_id}/suites")
    assert resp.status_code == 200, resp.reason
    assert len(resp.json()) == 3

    resp = client.get(f"/api/suite/result/{resp.json()[0]['id']}/cases")
    assert resp.status_code == 200, resp.reason
    assert len(resp.json()) == 3

    resp = client.get(f"/api/case/result/{resp.json()[0]['id']}/steps")
    assert resp.status_code == 200, resp.reason
    assert len(resp.json()) == 3
...
def test_update_run(client, db_with_3_runs):
    ...
    resp = client.patch(f"/api/run/{x.id}", json={"desc": "new description"})
    assert resp.status_code == 200
    assert resp.json()["desc"] == "new description"
    resp = client.get(f"/api/run/{x.id}")
    assert resp.status_code == 200
    assert resp.json()["desc"] == "new description"
