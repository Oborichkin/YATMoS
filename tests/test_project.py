...
def test_create_project(client):
    resp = client.post("/api/project/", json=project_x)
...
def test_get_one_project(client, db_with_3_projects):
    ...
    resp = client.get(f"/api/project/{x.id}")
    assert resp.status_code == 200
...
def test_update_project(client, db_with_3_projects):
    x, y, z = db_with_3_projects
    resp = client.patch(f"/api/project/{x.id}", json={"title": "Project XXX"})
    assert resp.status_code == 200
    assert resp.json()["title"] == "Project XXX"
    resp = client.patch(f"/api/project/{y.id}", json={"desc": "New description"})
    assert resp.status_code == 200
...
def test_delete_project(client, db_with_3_projects):
    x, y, z = db_with_3_projects
    resp = client.delete(f"/api/project/{x.id}")
    assert resp.status_code == 200
    resp = client.get("/api/projects")
    assert resp.status_code == 200
